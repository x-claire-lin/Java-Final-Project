...
<table border="1">
    ...
    <tr>
        <td><%= donation.getUserID()%>
        </td>
        <td><%= donation.getDonationCompany() %>
        </td>
        <td><%= donation.getProductID()%>
        </td>
        <td><%= donation.getProductName() %>
        </td>
        <td><%= donation.getDonationAmount() %>
        </td>
        <td><input type="number" id="quantity_<%= donation.getProductID() %>" min="0" step="0.001"></td>
        <td>
            <button onclick="submitForm('<%= donation.getProductID() %>', '<%= donation.getDonationAmount() %>')">
                Accept
            </button>
        </td>
    </tr>
